...
try:
    ...
    from .test_parametric import TestParametricROM
except ImportError:
    from test_nonparametric import TestROM
    from test_parametric import TestParametricROM
...
class TestBayesianParametricROM(TestParametricROM):
    """Test roms._bayes.BayesianParametricROM."""

    ROM = _module.BayesianParametricROM
    check_regselect_solver = False
    kwargs = {"num_posterior_draws": 2}
    ...
    def test_init(self):
        model = opinf.models.InterpContinuousModel("A")
        with pytest.raises(AttributeError) as ex:
            self.ROM(model)
        assert ex.value.args[0] == (
